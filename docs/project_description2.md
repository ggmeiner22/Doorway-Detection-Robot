...
3.  **Online Evaluation:** The robot performed four autonomous trials. Completion success was defined as detecting all three doors, surpassing the expected-reward threshold (`tau = 0.8`), and executing the return maneuver.

### Results
The experimental evaluation demonstrates that the POMDP-PID hybrid architecture effectively enables autonomous door counting and navigation. The system achieved a **91.7% door detection rate** across four trials, successfully identifying all three doorways in three of four tests. Successful navigation completion—passing three doors and returning to start—was achieved in **75%** of trials.
...
This is clearly seen in the proportional feature `PIDP9`. In `Wall` states, the probability mass is heavily concentrated in the central bins (4 and 5), summing to approximately **0.87** (0.41 and 0.46 respectively). In contrast, during the `Door_Start` state, the distribution shifts significantly toward lower values, with bins 2 and 3 accounting for **0.70** of the probability. This distinct shift captures the controller's immediate reaction to the discontinuity in the wall, serving as a reliable proprioceptive cue for the POMDP.

#### 3. Feature Discriminability (IR9)
The discretized IR features provided distinct signatures for the hallway macro-states. For example, `IR9`:
*   **Wall:** Highly stable, concentrated in central bins 5 and 6 (Sum approx. 0.84). Reflects a consistent 6cm distance.
*   **Door:** Distribution flattens and shifts to higher values (Bins 7, 8, 9 sum approx. 0.65), capturing the widening space.

#### 4. Online Door Detection & Expected Reward
...
The `Wall_End` state was uniquely characterized by the reward signal. Analysis of `cpt_Reward.csv` shows the probability of receiving a high reward given `Wall_End` is approx. 0.997, versus near zero for other states.
