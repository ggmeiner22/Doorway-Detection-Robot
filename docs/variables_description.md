...
*   **Processing:** Raw analog values are linearized into centimeters using a logarithmic calibration model derived from empirical data: `d_cm = A + B * ln(raw)`.
...
*   **Integral (I):** The accumulated error is clamped to a range of `+/- 0.5 * d_set` before being mapped to **10 bins**. This feature is particularly sensitive to the "open" space of a doorway.
*   **Derivative (D):** The rate of change is clamped to `+/- 0.1 * d_set` and mapped to **10 bins**. This captures the high-frequency edges of the doorframe.
